		...
		D: de::Deserializer<'de>,
	{
			...
			E: Clone + Deserialize<'de>,
		{
					...
					A: de::SeqAccess<'de>
			{
				...
				for (u, v, e) in edges {
					let un = g.get(&u).ok_or_else(|| de::Error::custom(&format!("Can't connect {} => {} because {} doesn't exist!", u, v, u)))?;
					let vn = g.get(&v).ok_or_else(|| de::Error::custom(&format!("Can't connect {} => {} because {} doesn't exist!", u, v, v)))?;
